...
class _HomeScreenState extends State<HomeScreen> {
      ...
      body: Row(children: [
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _SidebarItem(icon: Icons.picture_as_pdf_rounded, label: 'Create PDF', active: false, onTap: () => CreatePdfScreen.show(context), color: DS.cyan),
            _SidebarItem(icon: Icons.build_rounded, label: 'PDF Tools', active: false, onTap: () { /* navigate to tools */ }, color: DS.purple),
  ...
  Widget _webBody() {
    switch (_tab) {
      case 0: return _WebHome(recentPdfs: _recentPdfs, onOpenPdf: _pickPdf,
  ...
  Widget _buildMobileLayout() {
    final tabs = [(Icons.house_rounded, 'Home'), (Icons.access_time_rounded, 'Recents'), (Icons.auto_awesome_rounded, 'Templates'), (Icons.settings_rounded, 'Settings')];
    return Scaffold(
      backgroundColor: DS.bg,
      body: IndexedStack(index: _tab, children: [
        _MobileHome(onOpenPdf: _pickPdf, onScan: () => ScannerScreen.show(context), onNewDoc: () => DocumentEditorScreen.openNew(context), onCreatePdf: () => CreatePdfScreen.show(context), recentPdfs: _recentPdfs, onOpenRecent: (e) => _openPdf(e.virtualPath, bytes: e.bytes), onRemove: (e) => setState(() => _recentPdfs.remove(e))),
...
// ═══════════════════════════════════════════════════════════════
// WEB HOME — with animated drop zone + stats bar
// ═══════════════════════════════════════════════════════════════
...
class _WebHome extends StatelessWidget {
  ...
  const _WebHome({required this.recentPdfs, required this.onOpenPdf, required this.onOpenRecent, required this.onRemove});
...
class _QuickActionState extends State<_QuickAction> {
  bool _hover = false;
  @override
  Widget build(BuildContext context) => MouseRegion(onEnter: (_) => setState(() => _hover = true), onExit: (_) => setState(() => _hover = false),
    child: AnimatedContainer(duration: const Duration(milliseconds: 130), width: 140, height: 72, padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: _hover ? DS.bgHover : DS.bgCard, borderRadius: BorderRadius.circular(12), border: Border.all(color: _hover ? widget.color.withOpacity(0.3) : DS.separator, width: 0.8)),
      child: Row(children: [Icon(widget.icon, color: widget.color, size: 20), const SizedBox(width: 10), Expanded(child: Text(widget.label, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)))])));
}
...
class _FileRowState extends State<_FileRow> {
  ...
  Widget build(BuildContext context) => MouseRegion(onEnter: (_) => setState(() => _hover = true), onExit: (_) => setState(() => _hover = false),
    child: GestureDetector(onTap: widget.onTap,
      child: AnimatedContainer(duration: const Duration(milliseconds: 130), margin: const EdgeInsets.only(bottom: 4), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        decoration: BoxDecoration(color: _hover ? DS.bgHover : DS.bgCard, borderRadius: BorderRadius.circular(10), border: Border.all(color: _hover ? DS.separatorLight : DS.separator, width: 0.5)),
        child: Row(children: [
          ...
          const SizedBox(width: 12), Expanded(child: Text(widget.entry.displayName, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
          if (_hover) DSIconBtn(icon: Icons.close_rounded, tooltip: 'Remove', onTap: widget.onRemove, color: DS.textTertiary, size: 15),
        ]))));
}

// ═══════════════════════════════════════════════════════════════
// WEB RECENTS & TEMPLATES
// ═══════════════════════════════════════════════════════════════
...
class _MobileHome extends StatelessWidget {
  ...
  final List<_RecentEntry> recentPdfs; final ValueChanged<_RecentEntry> onOpenRecent, onRemove;
  const _MobileHome({required this.onOpenPdf, required this.onScan, required this.onNewDoc, required this.onCreatePdf, required this.recentPdfs, required this.onOpenRecent, required this.onRemove});

  @override
  Widget build(BuildContext context) => SafeArea(child: CustomScrollView(physics: const BouncingScrollPhysics(), slivers: [
    SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 24, 20, 20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [GradientText('DocSign', style: DS.display(size: 32)), const Spacer(), const DSBadge(text: 'v2.0', color: DS.indigo)]),
      const SizedBox(height: 4), Text('Professional PDF tools', style: DS.body(size: 14))]))),
    SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 16), child: PrimaryButton(label: 'Open a PDF', icon: Icons.folder_open_rounded, onTap: onOpenPdf, height: 52))),
    SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 0), child: GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 2, mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: 2.2, children: [
    ...
    if (recentPdfs.isEmpty) SliverFillRemaining(hasScrollBody: false, child: Center(child: Padding(padding: const EdgeInsets.all(40), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.inbox_rounded, size: 48, color: DS.textMuted), const SizedBox(height: 16), Text('No documents yet', style: DS.title(size: 16)), const SizedBox(height: 6), Text('Open a PDF to get started', style: DS.body(size: 13), textAlign: TextAlign.center)]))))
    else ...[SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(20, 28, 20, 12), child: Row(children: [Text('Recent', style: DS.title(size: 18)), const SizedBox(width: 8), Text('${recentPdfs.length}', style: DS.label(color: DS.textSecondary))]))),
      SliverList.builder(itemCount: recentPdfs.length, itemBuilder: (_, i) { final e = recentPdfs[i]; return _MobileFileRow(entry: e, onTap: () => onOpenRecent(e), onRemove: () => onRemove(e)); }),
